...
struct AppearanceStateEntities {
    mode: Entity<SelectState<Vec<AppearanceModeOption>>>,
    light_theme: Entity<SelectState<Vec<ThemeOption>>>,
    dark_theme: Entity<SelectState<Vec<ThemeOption>>>,
    font_family: Entity<SelectState<Vec<FontOption>>>,
...
fn appearance_options(cx: &App) -> AppearanceOptions {
    let modes = vec![
    ...
    (modes, light_themes, dark_themes, font_families, font_sizes)
}
...
fn appearance_states(
    ...
    window: &mut Window,
    cx: &mut Context<'_, AppearanceControls>,
) -> AppearanceStateEntities {
    let (modes, light_themes, dark_themes, font_families, font_sizes) = options;
...
impl AppearanceControls {
    pub(super) fn new(
        ...
        let mut controls = Self {
            ...
            font_family: states.font_family,
            font_size: states.font_size,
    ...
    fn reset(&self, window: &mut Window, cx: &mut Context<'_, Self>) {
        self.owner.update(cx, CadenceView::reset_appearance).ok();
        self.mode.update(cx, |state, cx| {
...
impl Render for AppearanceControls {
    fn render(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
        let (appearance, interactive) = self.owner.upgrade().map_or_else(
            || (AppearancePreferences::default(), false),
            |owner| {
